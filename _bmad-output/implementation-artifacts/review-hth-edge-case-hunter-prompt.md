...
Use the `bmad-review-edge-case-hunter` skill. You receive the diff below and may inspect the project read-only. Walk changed branching paths and boundary conditions. Return only concrete unhandled edge cases, each with severity and exact changed file/line area when possible.

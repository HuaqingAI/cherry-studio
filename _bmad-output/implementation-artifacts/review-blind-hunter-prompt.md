...
Use the `bmad-review-adversarial-general` skill. You receive only the diff below. No project context, no spec. Review adversarially for likely bugs, regressions, missing tests, or risky assumptions. Return findings only, each with severity and the exact changed file/line area when possible.
